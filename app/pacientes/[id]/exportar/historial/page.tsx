...
export default function ExportarHistorialPage() {
  ...
  return (
    <>
      ...
      <div className="flex items-center gap-4 p-6 no-print">
        ...
        <h1 className="text-xl font-bold flex-1">Exportar Historial Clínico</h1>
      ...
      <div id="export-print" className="max-w-3xl mx-auto border rounded-lg p-8 bg-white text-gray-900 space-y-5 mx-6 mb-10">
        ...
        <div className="flex items-start gap-4 border-b pb-4">
          ...
          <div className="text-right text-xs text-gray-500">
            <p className="font-semibold text-sm">HISTORIAL CLÍNICO</p>
        ...
        <Section title="Datos del Paciente">
          <div className="grid grid-cols-2 gap-x-8 gap-y-1">
            ...
            <Row label="Teléfono" value={patient.phone || patient.secondary_phone} />
          </div>
          {record?.consultation_date && (
            <Row label="Fecha de consulta" value={fmtDate(record.consultation_date)} />
          )}
          {(record?.reason_of_visit?.length > 0) && (
            <Row label="Motivo de consulta" value={
              [...(record.reason_of_visit ?? []), record.reason_other].filter(Boolean).join(", ")
            } />
          )}
        </Section>
        ...
        <Section title="Historia Médica">
          ...
          {med.bleeds_excessively && <Row label="Sangrado excesivo" value="Sí" />}
        ...
        {extraOral && Object.keys(extraOral).length > 0 && (
          <Section title="Examen Extra Oral">
            {extraOral.atm && <Row label="ATM" value={trueLabelList(extraOral.atm, {
              pain_palpation: "Dolor a la palpación", pain_opening: "Dolor en apertura", pain_closing: "Dolor en cierre",
              joint_noise: "Ruido articular", no_issues: "Sin molestias",
            })} />}
        ...
        {intraOral && Object.keys(intraOral).length > 0 && (
          <Section title="Examen Intra Oral">
            {intraOral.gums && <Row label="Encías" value={trueLabelList(intraOral.gums, {
              ...
              healthy: "Encía sana", periodontal_pockets: "Bolsas periodontales",
            })} />}
          ...
          <Section title="Recetas Emitidas">
            <table className="w-full text-sm border-collapse">
              <thead>
                <tr className="border-b border-gray-300">
                  <th className="text-left py-1 pr-4 font-semibold text-gray-700 w-28">Fecha</th>
                  <th className="text-left py-1 pr-4 font-semibold text-gray-700">Diagnóstico</th>
              ...
              <tbody className="divide-y divide-gray-100">
                {prescriptions.map((rx) => (
                  <tr key={rx.id}>
                    <td className="py-1 pr-4 align-top">{fmtDate(rx.date)}</td>
                    <td className="py-1 pr-4 align-top">{rx.diagnosis || "—"}</td>
